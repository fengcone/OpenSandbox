...
def test_volume_with_pvc_backend() -> None:
    vol = Volume(
        name="models",
        pvc=PVCBackend(claimName="shared-models"),
...
def test_volume_serialization_uses_aliases() -> None:
    vol = Volume(
        name="test",
        pvc=PVCBackend(claimName="my-pvc"),
        mountPath="/mnt/test",
        accessMode="RO",
        subPath="sub",
    )
