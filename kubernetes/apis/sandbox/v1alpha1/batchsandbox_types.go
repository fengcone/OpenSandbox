...
type BatchSandboxSpec struct {
	...
	// nil = no operation (initial state)
	// true = request Pause
	// false = request Resume
	// After the operation completes, Controller clears it to nil to reset the signal line.
	// +optional
...
type BatchSandboxStatus struct {
	...
	// Message provides human-readable status information (e.g., failure reason when Phase=Failed).
	// +optional
	Message string `json:"message,omitempty"`
