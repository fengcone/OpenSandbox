...
var _ = Describe("PauseResume", Ordered, func() {
	...
	Context("Pause and Resume", func() {
		It("should complete the full pause-resume flow via spec.pause trigger", func() {
			...
			By("waiting for BatchSandbox to be Running (phase=Running)")
			Eventually(func(g Gomega) {
			...
			// Verify spec.pause cleared to null
			By("verifying spec.pause is cleared after pause completes")
			cmd = exec.Command("kubectl", "get", "batchsandbox", sandboxName,
				"-n", pauseResumeNamespace, "-o", "jsonpath={.spec.pause}")
			output, err = utils.Run(cmd)
			Expect(err).NotTo(HaveOccurred())
			Expect(output).To(BeEmpty(), "spec.pause should be cleared to null after pause completes")
	...
	Context("Pause Resume Failure", func() {
		...
		It("should set Phase=Running+PauseFailed when commit/push fails with invalid registry", func() {
			...
			cmd = exec.Command("kubectl", "patch", "secret", "registry-push-secret", "-n", pauseResumeNamespace,
			...
			By("waiting for BatchSandbox to be Running with PauseFailed condition")
			Eventually(func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "batchsandbox", sandboxName,
					"-n", pauseResumeNamespace, "-o", "jsonpath={.status.phase}")
				output, err := utils.Run(cmd)
		...
		It("should set Phase=Failed+ResumeFailed when resume fails with invalid image", func() {
			...
			cmd = exec.Command("kubectl", "patch", "sandboxsnapshot", sandboxName,
			...
			By("verifying SandboxSnapshot is deleted")
			cmd = exec.Command("kubectl", "get", "sandboxsnapshot", sandboxName,
				"-n", pauseResumeNamespace, "-o", "name")
			output, err = utils.Run(cmd)
			Expect(err).To(HaveOccurred(), "SandboxSnapshot should be deleted")
		...
		It("should successfully retry pause after fixing registry config", func() {
			...
			By("verifying Phase is still Running (retryable)")
			cmd = exec.Command("kubectl", "get", "batchsandbox", sandboxName,
				"-n", pauseResumeNamespace, "-o", "jsonpath={.status.phase}")
			output, err := utils.Run(cmd)
			Expect(err).NotTo(HaveOccurred())
			...
			By("triggering retry pause (should clear PauseFailed and succeed)")
			cmd = exec.Command("kubectl", "patch", "batchsandbox", sandboxName,
				"-n", pauseResumeNamespace, "--type=merge",
				"-p", `{"spec":{"pause":true}}`)
			_, err = utils.Run(cmd)
